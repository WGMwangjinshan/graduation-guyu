    ...
    <el-table :data="tableData" style="width: 100%">
      ...
      <el-table-column fixed="right" label="操作" width="300">
        <template slot-scope="scope">
          ...
          >编辑文档</el-button>
          <el-button type="text" size="small" @click.native.prevent="delFile(scope.$index, tableData)">删除文档</el-button>
        </template>
      </el-table-column>
    </el-table>

    <el-dialog title="编辑文档" :visible.sync="contentDialog" width="80%" height="80%" center>
      <el-input type="textarea" :rows="20" placeholder="请输入内容" v-model="content_txt" v-if="contentType == 'txt'"></el-input>
      <!-- 如果是表格的话 -->
      <!-- <el-table
      :data="content_table"
      style="width: 100%"
        ...
        .then(res => {
          ...
          if (this.status == "success") {
            ...
            if (
              ...
              this.content_table = eval("(" + jsonObj + ")").data.table_content.table_cells
              this.line_len = eval("(" + jsonObj + ")").data.table_content.line_len;
              this.row_len = eval("(" + jsonObj + ")").data.table_content.row_len;
